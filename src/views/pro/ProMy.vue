        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <el-tag v-show="scope.row.harry === '正常'" type="success" disable-transitions>
                        {{ scope.row.harry }}
                    </el-tag>
                    <el-tag v-show="scope.row.harry === '重要'" type="warning" disable-transitions>
                        {{ scope.row.harry }}
                    </el-tag>
                    <el-tag v-show="scope.row.harry === '紧急'" type="danger" disable-transitions>
                        {{ scope.row.harry }}
                    </el-tag>
                </template>
            </el-table-column>
            ...
            <el-table-column
                ...
                label="操作">
                <template slot-scope="scope">
                    <el-button style="padding: 3px" size="mini" @click="showProcessDetail(scope.row)" type="primary">
                        查看
                    </el-button>
                    <el-button :disabled="judgeDelete(scope.row)"
                               @click="deleteMyProcess(scope.row)"
        ...
        <el-dialog
            ...
            <div>
                ...
                <el-descriptions title="报销详情" v-bind="bursementRecord"
                    ...
                    <el-descriptions-item label="报销总金额" v-if="bursementRecord.allMoney !== null">
                        {{ bursementRecord.allMoney }}元
                    </el-descriptions-item>
                    <el-descriptions-item label="票据总数" v-if="bursementRecord.allinvoices !== null">
                        {{ bursementRecord.allinvoices }}张
                    </el-descriptions-item>
                    <el-descriptions-item label="报销方式" v-if="burseType !== null">{{ burseType }}</el-descriptions-item>

                </el-descriptions>
                <el-table size="mini" stripe v-if="bursementRecord.details !== null"
                          v-show="processDetail.typeName === '费用报销'"
...
export default {
    ...
    methods: {
        ...
        initMyProcessList() {
            ...
            this.userId = obj.id
            this.getRequest('/processList/findMyApply?rows=' + this.rows + '&pageSize=' + this.pageSize + '&userId=' + this.userId).then(resp => {
                if (resp) {
                    this.myProcessList = resp.data
        ...
        deleteMyProcess(data) {
            this.$confirm('此操作将删除您的' + data.typeName + ',是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.putRequest('/processList/deleteMyProcess?processId=' + data.processId).then(resp => {
                    if (resp) {
                        this.initList();
                    }
                })
